package com.salesforce.pagefactory;
...
public class PageFactory {
	...
	static {
		try {
			...
			getClassInPackage(new PropertyReader().getApplicationproperty("page.packages"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}

	public SalesForceBasePage initialize(String pageName) {
		...
		try {
			...
			if (packageName != null) {
				String pageClassFullName = packageName + "."
						+ formattedPageName;
				page = (SalesForceBasePage) Class.forName(pageClassFullName)
						.newInstance();
			} else {
				System.out.println("Page Class with" + formattedPageName + "doesnt exist .Please provide valid page Name");
			}
